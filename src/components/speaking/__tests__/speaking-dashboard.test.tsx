...
describe('SpeakingDashboard', () => {
  it('renders summary metrics, study focus, and resume links', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /track part coverage and recent speaking confidence/i }),
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole('link', { name: /return to speaking practice/i })).toHaveAttribute(
      'href',
      '/speaking',
    );
    ...
    expect(resumeLinks[0]).toHaveAttribute(
      'href',
      `/speaking?promptId=${sampleSpeakingRecentSessions[0]!.promptId}&sessionId=${sampleSpeakingRecentSessions[0]!.sessionId}`,
    );
  });
});
